...
3. Confirm the provider root is available through the named environment variable or an explicitly supplied local config outside the repository.
...
Omit `--local-config` only when the configured root environment is already sufficient. Optional title, domain, and ASCII slug arguments may be added only when the user supplied them.
