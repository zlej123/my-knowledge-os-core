...
지식과 투자 판단을 하나의 읽기 전용 화면에서 보려면 다음 명령을 실행합니다.

```bash
mko ui
```

기본 주소는 `http://127.0.0.1:2036`입니다. UI 서버는 loopback 주소에만 바인딩되며 검색과
현재 상태 조회만 제공합니다. 검토·승인·발행·포트폴리오 결정·주문을 변경하는 endpoint는
없고, 필요한 다음 행동은 기존 owner-controlled 명령으로 안내만 합니다.

Thesis는 전환 기간 동안 별도 localhost read API를 읽습니다. 기본 주소는
`http://127.0.0.1:2035`이며 다른 loopback 주소는 `--thesis-url`로 지정할 수 있습니다.
인증이 필요한 경우 `THESIS_API_TOKEN`을 `mko ui` 프로세스 환경에 설정합니다. 토큰은
브라우저로 전달되지 않습니다. Thesis가 꺼져 있거나 인증되지 않아도 MKO 지식 화면은 계속
사용할 수 있습니다.
